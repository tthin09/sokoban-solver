...
def convert_image_to_map(image_path: str, DEBUG_MODE=False) -> Map:
    ...
    Returns:
        tuple[np.ndarray, list]: A tuple containing
            - The 2D map as a NumPy array
            - A list of (x, y) coordinates for all destinations
    """
    # Map tile position
    tile_loc = {
        "wall": [],
    ...
    if DEBUG_MODE: print(f'Map size: {map_size[0]}x{map_size[1]}')
    ...
    for wall_center in tile_loc['wall']:
        ...
        wall_count += 1
    if DEBUG_MODE: print(f'Wall count: {wall_count}')
